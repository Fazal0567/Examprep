      ...
      {!activeQuiz && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1 p-1 bg-slate-100/90 rounded-2xl border border-slate-200/80 w-full sm:w-auto overflow-x-auto">
              <button
                onClick={() => setActiveTab('create')}
                className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                  activeTab === 'create'
                ...
                <div className="bg-white rounded-3xl p-4 sm:p-6 md:p-8 border border-slate-200/80 shadow-xs space-y-5 sm:space-y-6">
                    ...
                    {activeQuiz.questions[currentQuestionIdx].options.map((opt, optIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-3 sm:p-4 rounded-2xl border text-xs text-left transition-all flex items-center justify-between gap-3 ${buttonStyle}`}
                        >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100 gap-2 flex-wrap sm:flex-nowrap">
                    <button
                      ...
                      className="px-3.5 sm:px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 disabled:opacity-30 hover:bg-slate-50 flex items-center gap-1"
                    >
                    ...
                    {!isSubmitted ? (
                      currentQuestionIdx === activeQuiz.questions.length - 1 ? (
                        <button
                          ...
                          className="px-5 sm:px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-md shadow-emerald-200 transition-all flex items-center gap-2"
                        >
                        ...
                        <button
                          onClick={() => setCurrentQuestionIdx((p) => p + 1)}
                          className="px-4 sm:px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-xs flex items-center gap-1"
                        >
                      ...
                      currentQuestionIdx < activeQuiz.questions.length - 1 ? (
                        <button
                          onClick={() => setCurrentQuestionIdx((p) => p + 1)}
                          className="px-4 sm:px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-bold flex items-center gap-1 hover:bg-indigo-700"
                        >
                          <span>Next Question</span>
                          <ChevronRight className="w-3.5 h-3.5" />
                        </button>
